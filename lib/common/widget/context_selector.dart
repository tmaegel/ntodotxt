  ...
  Widget build(BuildContext context) {
    return BlocBuilder<FilterCubit, FilterState>(
      buildWhen: (FilterState previousState, FilterState state) =>
          previousState.filter.contexts != state.filter.contexts,
      builder: (BuildContext context, FilterState state) {
        if (items.isEmpty) {
          return Text(
            'No context tags available',
          ...
          return Wrap(
            ...
            children: [
              for (var i in items.toList()..sort())
                ChoiceChip(
                  label: Text(i),
                  selected: state.filter.contexts.contains(i),
                  showCheckmark: false,
                  ...
                  onSelected: (bool selected) {
                    if (multiSelectionEnabled) {
                      if (state.filter.contexts.contains(i)) {
                        if (emptySelectionAllowed ||
                            state.filter.contexts.length > 1) {
                          context.read<FilterCubit>().removeContext(i);
                        }
                      } else {
                        context.read<FilterCubit>().addContext(i);
                      }
                    } else {
                      if (selected) {
                        context.read<FilterCubit>().updateContexts({i});
                      } else {
                        if (emptySelectionAllowed) {
                          context.read<FilterCubit>().removeContext(i);
                        }
